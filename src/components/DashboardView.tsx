...
const DEFAULT_VISIBLE = ['cpu', 'ram', 'gpu', 'disk', 'net', 'home', 'game', 'guardian', 'creative', 'power', 'chart', 'activity']

const SECTION_DEFS = [
  { id: 'metrics',  label: 'Resource Metrics' },
  { id: 'control',  label: 'Agentic Control' },
  { id: 'creative', label: 'Modules Suite' },
      ...
      {sectionOrder.map(sectionId => {
        ...
        if (sectionId === 'control') return (
          <section key="control" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ fontSize: '9px', fontWeight: 'bold', color: '#3f3f46', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '4px' }}>// Agentic_Control</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gap: '16px' }}>
              {show('game') && (
                <div className="v-card" style={{ padding: '20px', display: 'flex', gap: '20px', alignItems: 'center', background: gameMode ? 'linear-gradient(135deg, rgba(239,68,68,0.1) 0%, rgba(8,13,23,0.9) 100%)' : 'rgba(255,255,255,0.02)', border: gameMode ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(255,255,255,0.05)', boxShadow: gameMode ? '0 0 30px rgba(239,68,68,0.1)' : 'none', transition: 'all 0.5s cubic-bezier(0.4, 0, 0.2, 1)' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#52525b', marginTop: '4px' }}>{gameMode ? 'AI isolated to CCD2. SCX active.' : 'Prioritize L3 cache for gaming.'}</div>
                  </div>
              ...
              {show('guardian') && (
                <div className="v-card" style={{ padding: '20px', display: 'flex', gap: '20px', alignItems: 'center', background: guardianEnabled ? 'linear-gradient(135deg, rgba(16,185,129,0.05) 0%, rgba(8,13,23,0.9) 100%)' : 'rgba(255,255,255,0.02)', border: guardianEnabled ? '1px solid rgba(16,185,129,0.2)' : '1px solid rgba(255,255,255,0.05)', transition: 'all 0.3s ease' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '10px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.12em', color: guardianEnabled ? 'var(--signal)' : '#71717a', fontWeight: 'bold', marginBottom: '4px' }}>Agentic Guardian</div>
        ...
        if (sectionId === 'creative' && (show('creative') || show('home'))) return (
          <section key="creative" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ fontSize: '9px', fontWeight: 'bold', color: '#3f3f46', textTransform: 'uppercase', letterSpacing: '0.2em', marginLeft: '4px' }}>// Modules_Suite</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {show('home') && (
                <div className="v-card" style={{ padding: '20px', display: 'flex', gap: '20px', alignItems: 'center', background: 'linear-gradient(135deg, rgba(6,182,212,0.05) 0%, rgba(8,13,23,0.9) 100%)' }}>
                  <div style={{ padding: '12px', borderRadius: '12px', background: 'rgba(34,211,238,0.1)', border: '1px solid rgba(34,211,238,0.2)', color: '#22d3ee' }}><Home size={24} /></div>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#52525b', marginTop: '4px' }}>Monitor and automate PC-to-Home triggers.</div>
                  </div>
                  <button onClick={() => onNavigate?.('home')} style={{ padding: '10px 20px', borderRadius: '8px', background: '#0891b2', border: 'none', color: 'white', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 0 15px rgba(8,145,178,0.3)' }}>DASHBOARD</button>
                </div>
              )}
              {show('creative') && (
                <div className="v-card" style={{ padding: '20px', display: 'flex', gap: '20px', alignItems: 'center', background: 'linear-gradient(135deg, rgba(168,85,247,0.05) 0%, rgba(8,13,23,0.9) 100%)' }}>
                  <div style={{ padding: '12px', borderRadius: '12px', background: 'rgba(168,85,247,0.1)', border: '1px solid rgba(168,85,247,0.2)', color: '#a855f7' }}><Sparkles size={24} /></div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '10px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.12em', color: '#71717a', fontWeight: 'bold', marginBottom: '4px' }}>Creative Suite</div>
                    <div style={{ fontSize: '14px', fontWeight: 'bold', color: 'white' }}>Headless ComfyUI Core</div>
                    <div style={{ fontSize: '11px', color: '#52525b', marginTop: '4px' }}>Neural synthesis engine active and connected.</div>
                  </div>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => onNavigate?.('image-gen')} style={{ padding: '8px 16px', borderRadius: '6px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>IMAGE</button>
                    <button onClick={() => onNavigate?.('video-gen')} style={{ padding: '8px 16px', borderRadius: '6px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'white', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>VIDEO</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="v-card" style={{ padding: 0, overflow: 'hidden' }}>
              ...
              {auditLog.length === 0 ? (
                ...
                <div key={entry.id} style={{ display: 'grid', gridTemplateColumns: '28px 60px 1fr 50px', gap: '0 10px', padding: '7px 16px', borderBottom: '1px solid rgba(255,255,255,0.02)', fontSize: '10px', fontFamily: 'monospace', alignItems: 'center', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.003)' }}>
                  ...
                  {entry.source === 'terminal' ? <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--signal)', fontSize: '8px', textTransform: 'uppercase' }}><Terminal size={8} /> Term</div> : <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--crimson)', fontSize: '8px', textTransform: 'uppercase' }}><Bot size={8} /> AI</div>}
