...
import { Cpu, MemoryStick, HardDrive, Network, Settings2, Eye, EyeOff, Terminal, CheckCircle2, XCircle, Bot, Activity, Zap, Sparkles, Home } from 'lucide-react'
...
const DEFAULT_VISIBLE = ['cpu', 'ram', 'gpu', 'disk', 'net', 'home', 'creative', 'power', 'chart', 'activity']
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {customize && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          className="v-card" style={{ padding: '14px 18px', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          {WIDGET_DEFS.map(w => (
      ...
      {/* Creative Suite Widget */}
      {show('creative') && (
        <div className="v-card" style={{ padding: '20px', display: 'flex', gap: '16px' }}>
          {/* ... existing content ... */}
        </div>
      )}
      ...
      {/* Smart Home Widget */}
      {show('home') && (
        <div className="v-card" style={{ padding: '20px', display: 'flex', gap: '20px', alignItems: 'center', background: 'linear-gradient(135deg, rgba(6,182,212,0.05) 0%, rgba(8,13,23,0.9) 100%)' }}>
          <div style={{ padding: '12px', borderRadius: '12px', background: 'rgba(34,211,238,0.1)', border: '1px solid rgba(34,211,238,0.2)', color: '#22d3ee' }}>
            <Home size={24} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '10px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.12em', color: '#71717a', fontWeight: 'bold', marginBottom: '4px' }}>Smart Home Hub</div>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: 'white' }}>Home Assistant Integration</div>
            <div style={{ fontSize: '11px', color: '#52525b', marginTop: '4px' }}>Monitor Ring cameras and automate PC-to-Home triggers.</div>
          </div>
          <button
            onClick={() => onNavigate?.('home')}
            style={{ padding: '10px 20px', borderRadius: '8px', background: '#0891b2', border: 'none', color: 'white', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 0 15px rgba(8,145,178,0.3)' }}
          >
            LAUNCH DASHBOARD
          </button>
        </div>
      ...
      {show('power') && (
        <div className="v-card" style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontSize: '10px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.15em', color: '#71717a', fontWeight: 'bold' }}>Power Management</span>
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
            {[
              { id: 'power-saver', label: 'Saver', icon: Sparkles, color: 'var(--signal)' },
              { id: 'balanced',    label: 'Balanced', icon: Activity, color: '#64748b' },
              { id: 'performance', label: 'Performance', icon: Zap, color: 'var(--crimson)' }
            ].map(p => {
              const active = powerProfile === p.id
              return (
                <button
                  key={p.id}
                  onClick={() => handleSetPowerProfile(p.id)}
                  style={{
                    flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', padding: '10px 4px', borderRadius: '8px',
                    background: active ? `${p.color}15` : 'rgba(255,255,255,0.02)',
                    border: `1px solid ${active ? p.color : 'rgba(255,255,255,0.05)'}`,
      ...
      {show('gpu') && stats?.gpu && (
        <div className="v-card" style={{ padding: '16px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
            ...
            <span style={{ fontSize: '10px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.15em', color: '#71717a', fontWeight: 'bold' }}>
              {stats.gpu.model}
            </span>
          </div>
      ...
      {show('chart') && (
        <div className="v-card" style={{ padding: '20px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
            <h3 style={{ margin: 0, fontSize: '10px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.15em', color: '#71717a', fontWeight: 'bold' }}>
              ...
              <div style={{ width: '100%', height: '80px', position: 'relative' }}>
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center' }}>
                  <svg viewBox={`0 0 200 40`} preserveAspectRatio="none" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
                    {cpuHist.length >= 2 && (() => {
                      const pts = cpuHist.map((v, i) => `${(i/(cpuHist.length-1))*200},${40-(v/100)*38-1}`).join(' ')
                      return <>
                        <polyline points={pts + ` 200,40 0,40`} fill="rgba(239,68,68,0.1)" stroke="none" />
                        <polyline points={pts} fill="none" stroke="var(--crimson)" strokeWidth="1.5" strokeLinejoin="round" />
                      </>
                    })()}
                    {ramHist.length >= 2 && (() => {
                      const pts = ramHist.map((v, i) => `${(i/(ramHist.length-1))*200},${40-(v/100)*38-1}`).join(' ')
                      return <>
                        <polyline points={pts + ` 200,40 0,40`} fill="rgba(34,211,238,0.06)" stroke="none" />
                        <polyline points={pts} fill="none" stroke="var(--signal)" strokeWidth="1.5" strokeLinejoin="round" strokeDasharray="4 2" />
                      </>
                    })()}
                    {gpuHist.length >= 2 && (() => {
                      const pts = gpuHist.map((v, i) => `${(i/(gpuHist.length-1))*200},${40-(v/100)*38-1}`).join(' ')
                      return <>
                        <polyline points={pts + ` 200,40 0,40`} fill="rgba(168,85,247,0.06)" stroke="none" />
                        <polyline points={pts} fill="none" stroke="#a855f7" strokeWidth="1.5" strokeLinejoin="round" strokeDasharray="2 3" />
                      </>
                    })()}
                  </svg>
                </div>
      ...
      {show('activity') && (
        <div className="v-card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontSize: '10px', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.12em', color: '#a1a1aa', fontWeight: 'bold' }}>Recent Commands</span>
          </div>
          {auditLog.length === 0 ? (
            ...
            <div key={entry.id} style={{ display: 'grid', gridTemplateColumns: '28px 60px 1fr 50px', gap: '0 10px', padding: '7px 16px', borderBottom: '1px solid rgba(255,255,255,0.02)', fontSize: '10px', fontFamily: 'monospace', alignItems: 'center', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.003)' }}>
              {entry.exit_code === 0 || entry.exit_code === null
                ? <CheckCircle2 size={12} style={{ color: 'var(--signal)' }} />
                : <XCircle size={12} style={{ color: 'var(--crimson)' }} />
              }
              {entry.source === 'terminal'
                ? <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--signal)', fontSize: '8px', textTransform: 'uppercase' }}><Terminal size={8} /> Term</div>
                : <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--crimson)', fontSize: '8px', textTransform: 'uppercase' }}><Bot size={8} /> AI</div>
              }
