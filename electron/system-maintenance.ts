...
export function setupMaintenanceHandlers(win: any) {
  ...
  // 14. log-remediation-analyze
  ipcMain.handle('log-remediation-analyze', async (_, { unit, lines, model }: { unit?: string; lines?: number; model?: string }) => {
    try {
      const safeLines = Math.max(10, Math.min(100, lines || 50))
      const safeUnit = unit ? unit.replace(/[^a-zA-Z0-9@._-]/g, '') : ''
      const logCmd = safeUnit
        ? `journalctl -u ${safeUnit} -n ${safeLines} --no-pager`
        : `journalctl -p 3 -n ${safeLines} --no-pager`

      let logs = ''
      try {
        const { stdout } = await execPromise(logCmd)
        logs = stdout.trim()
      } catch (err: any) {
      ...
      if (!logs) {
        return {
          ...
          diagnosis: 'No log entries were returned to analyze.',
          remediationCommand: null,
          remediationSafety: 'safe'
        }
      }

      const activeModel = model || 'qwen3:8b'
      const systemPrompt = `You are Vortex Log Diagnostics, an expert Linux system administrator AI.
Analyze the provided system logs and return a JSON object describing the problem, diagnosis, a remediation command, and safety.
Your response MUST be a single, valid JSON object and nothing else. No markdown wrapping (like \`\`\`json), no trailing text, no extra characters.
Format:
{
...
Rules:
- 'safe': commands like systemctl restart a-safe-service, check status.
- 'needs-confirmation': commands modifying configs, installing packages, stopping services.
- 'dangerous': commands deleting system files, altering disks, running curl | sh.
- Set remediationCommand to null if it cannot be solved via a simple command, or if it is dangerous.`
      ...
      try {
        ...
        return {
          ...
          remediationCommand: parsed.remediationCommand || null,
          remediationSafety: parsed.remediationSafety || 'safe'
        }
      } catch (jsonErr: any) {
        console.error('Failed to parse JSON from model output:', clean, jsonErr)
        return {
          success: false,
          error: `Failed to parse AI diagnostics output: ${jsonErr.message}. Raw output: ${text}`
        }
      }
    } catch (e: any) {
      return { success: false, error: e.message }
  ...
  // 15. log-remediation-apply
  ipcMain.handle('log-remediation-apply', async (_, { command }: { command: string }) => {
    if (!command) {
    ...
    const dangerousPatterns = [
      /\brm\s+-rf\s+\/\b/,
      /\brm\s+-rf\s+.*?\/(?:etc|var|usr|boot|dev|opt|sys|proc|home)\b/,
      /\bmkfs\b/,
      /\bdd\b.*?\bof=\/dev\//,
      />\s*\/dev\/sd/,
      /\bchown\b.*?\/\b/,
    ...
    if (dangerousPatterns.some(pat => pat.test(command))) {
